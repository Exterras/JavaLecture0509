package classes;

import java.awt.Button;
import java.awt.Color;
import java.awt.Font;
import java.awt.Frame;
import java.awt.GridLayout;
import java.awt.Panel;
import java.awt.TextField;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.awt.event.WindowAdapter;
import java.awt.event.WindowEvent;

public class AwtCalculator extends Frame implements ActionListener{

	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		
	}
	
	public AwtCalculator(){
		setLayout(new GridLayout(5, 1)); // 5, 1
		TextField textfield = new TextField();
		textfield.setFont(new Font("consolas", Font.BOLD, 50)); // font setting
		add(textfield); // Put in the GridLayout (1,1)
		
		Panel[] arrPanel = new Panel[4];
		// GridLayout (1,1)
		String[][] calculatorStr = {
				{ "7", "8", "9", "+" }, // GridLayout(2,1)
				{ "4", "5", "6", "-" }, // GridLayout(3,1)
				{ "1", "2", "3", "*" }, // GridLayout(4,1)
				{ "0", ".", "/", "=" }  // GridLayout(5,1)
		};
		
		for (int i = 0; i < calculatorStr.length; i++) {
			arrPanel[i] = new Panel();
			arrPanel[i].setLayout(new GridLayout(1, 4));
			for (int j = 0; j < calculatorStr[i].length; j++) {
				Button calculatorButton = new Button(calculatorStr[i][j]);
				calculatorButton.setFont(new Font("consolas", Font.BOLD, 40)); // font setting
				calculatorButton.setForeground(Color.BLUE);
				arrPanel[i].add(calculatorButton);
				// arrPanel[i] Put in the GridLayout (i, 1~4)
				String calculatorCharacter = calculatorStr[i][j];
				calculatorButton.addActionListener(new ActionListener() {
					
					@Override
					public void actionPerformed(ActionEvent e) {
						calculate();
					}
				});
			}
			add(arrPanel[i]);
		}// Put in the GridLayout (2~5, 1)
		
		setTitle("Calculator");
		setSize(400, 500);
		setVisible(true);

		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				System.exit(0);
			}
		}); // window terminate
	}
	
	public void calculate(){
		textfield.setText(calculatorCharacter);
	}
	
	public static void main(String[] args) {
		new AwtCalculator();
	}

}
